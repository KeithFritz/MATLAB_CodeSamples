...
% Single-Joint Robot (HW 10) [PD Controller]
...
for i = 2:n
    ...
    th(i) = th(i-1) + dth(i-1)*h;  % numerical inegratin to calculate position from velocity
    
    e(i) = thd(i)-th(i);        % define the position error
    de(i) = dthd(i)-dth(i);     % define the velocity error
    
    tau = kp*e(i) + kd*de(i);   % torque/force control
